...
from feapder.core.mixin import BatchDBMixinBase, BatchMysqlDBMixin
...
class BatchSpider(BatchParser, Scheduler, BatchDBMixinBase, metaclass=BatchMetaclass):
    def __init__(
        self,
        task_table,
        batch_record_table,
        ...
        @param db_type: 数据库类型，mysql或mongo 默认mysql
        ---------
        @result:
        """
        Scheduler.__init__(
        ...
        self._format_task_condition(task_condition)
        self._format_task_order(task_order_by)
        ...
        elif 1 > self._batch_interval >= 1 / 24:
    ...
    def start_monitor_task(self):
        ...
        self._create_batch_record_table()
        ...
        while True:
            try:
                ...
                if todo_task_count < self._min_task_count:  # 从mysql中取任务
                    ...
                    tasks = self.get_todo_task_from_db()
                    if not tasks:
                        # 状态为0的任务已经做完，需要检查状态为2的任务是否丢失
    
                        if todo_task_count == 0:
                            # 当redis中无待做任务时，如果此时数据库中存在状态为2的任务，即为丢失任务，应重做这部分任务
                            lose_task_count = self.get_lose_task_count()
                            
                            if not lose_task_count:
                                # 没有丢失的任务，则等待一段时间，等待任务执行完毕
                                time.sleep(self._check_task_interval)
                                continue
                            
                            elif lose_task_count > self._task_limit * 5:
                                # 丢失任务太多，直接重置，否则每次等redis任务消耗完再取下一批丢失任务，速度过慢
                                log.info("正在重置丢失任务为待做 共 {} 条".format(lose_task_count))
                                # 重置正在做的任务为待做
                                if self.reset_lose_task_from_db():
                                    log.info("重置丢失任务成功")
                                else:
                                    log.info("重置丢失任务失败")
                                continue
                                ...
                                tasks = self.get_doing_task_from_db()
    ...
    def __get_task_state_count(self):
        task_state_count = self._get_task_state_count()
        task_state = {
    ...
    def update_task_done_count(self):
        ...
        @result:
        """
        task_state = self.__get_task_state_count()
        # log.info('《%s》 批次进度 %s/%s' % (self._batch_name, done_task_count, total_task_count))
        self._update_batch_record(task_state)
    
    def update_is_done(self):
        self._update_batch_record_done()
    
    def get_todo_task_from_db(self):
        """
        @summary: 取待做的任务
        ---------
        ---------
        ...
        # 查询任务
        tasks = self._get_todo_task()
        
        if tasks:
            # 更新任务状态
            for i in range(0, len(tasks), 10000):  # 10000 一批量更新
                task_ids = [task[0] for task in tasks[i: i + 10000]]
                self._update_task_status(task_ids)
        
        return tasks
    
    def get_doing_task_from_db(self):
        """
        @summary: 取正在做的任务
        ---------
        ---------
    ...
    def get_lose_task_count(self):
        ...
        @return:
        """
        latest_batch_info = self._get_latest_batch_record()
        return latest_batch_info['total_count'] - latest_batch_info['done_count']
    
    def reset_lose_task_from_db(self):
        """
        @summary: 重置丢失任务为待做
        ---------
        ---------
    ...
    def check_batch(self, is_first_check=False):
        ...
        """
        batch_info = self._get_latest_batch_record()
        
        if batch_info:
            batch_date = batch_info['batch_date']
            total_count = batch_info['total_count']
            done_count = batch_info['done_count']
            ...
            if total_count == done_count:
                ...
                elif related_spider_is_done is True or related_spider_is_done is None:
                    # 更新is_done 状态
                    self.update_is_done()
                ...
                if time_difference >= datetime.timedelta(days=self._batch_interval):
                    ...
                    # 初始化任务表状态
                    if self.init_task() is not False:  # 更新失败返回False 其他返回True/None
                        # 初始化属性
                        self.init_property()
    ...
    def related_spider_is_done(self):
        ...
        if self._related_batch_record:
            batch_info = self._get_latest_batch_record()
            is_done = batch_info.get('is_done')
            is_done = is_done if is_done else None
    ...
    def record_batch(self):
        ...
        @result:
        """
        total_task_count = self._get_task_total_count()
        batch_date = tools.get_current_date(self._date_format)
        affect_count = self._add_batch_record(batch_date, total_task_count)
    ...
    def task_is_done(self):
        ...
        if batch_info:
            ...
            self._batch_date_cache = batch_info['batch_date']
            total_count = batch_info['total_count']
            done_count = batch_info['done_count']
            is_done = batch_info['is_done']
        ...
        if is_done:  # 检查任务表中是否有没做的任务 若有则is_done 为 False
            ...
            with RedisLock(
                ...
                if lock.locked:
                    ...
                    tasks = self._get_done_or_doing_tasks(limit=1)
                    if tasks:
                        log.info("检测到任务表中有未完成任务，等待任务下发")
                        is_done = False
                        self._update_batch_record_done_cache_date()
